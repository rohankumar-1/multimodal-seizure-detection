"""
SupFusion-style training: EEG + ECG encoders, concat fusion, classification head.

Uses `multimodal` (external package): MultimodalModel, ConcatFusion, ClassificationTask, Trainer.

Edit constants at the top. Data: processed .npz with keys `eeg` (N, 2, T), `ecg` (N, 1, T), `binary_label` (N,).

Press Ctrl+C during `trainer.train` to stop early and continue to threshold + test eval.

Run: python supfusion.py
"""
...
from multimodal.fusion import ConcatFusion, AdditiveFusion, BilinearFusion
...
# ConcatFusion expects a list of tensors; MultimodalModel only builds that list if you set:
#   fusion_modality_order — otherwise _fuse passes the encoded dict and torch.cat can break.
...
OUT_DIR = "runs/supfusion"
CKPT_PATH = os.path.join(OUT_DIR, "supfusion_last.pt")
RESULTS_JSON = "results/supfusion_additive.json"
# ------------------


class MLP(nn.Module):
    """Classification head after fusion (same role as template)."""
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        return {'classification': self.fc2(x)}
...
def _unwrap_predictions(raw: Any) -> torch.Tensor:
    """MultimodalModel.forward returns (preds, embeddings); head may return dict with 'classification'."""
    if isinstance(raw, tuple):
        raw = raw[0]
    if isinstance(raw, dict):
        if "classification" in raw:
            return cast(torch.Tensor, raw["classification"])
        return cast(torch.Tensor, next(iter(raw.values())))
...
@torch.no_grad()
def find_best_threshold_youden(model: nn.Module, val_loader: DataLoader, device: str) -> float:
    ...
    for batch in val_loader:
        ...
        logits = forward_predictions(model, batch, device, LABEL_KEY_IN_BATCH)
...
def evaluate_binary_threshold(
    ...
    """Metrics on dict batches (eeg, ecg, y); uses P(class=1) vs threshold (chrononet-style reporting)."""
    model.eval()
    all_labels: list[torch.Tensor] = []
...
def build_loaders() -> tuple[DataLoader, DataLoader, DataLoader]:
    """Train, val, test — same `SupFusionDictDataset` + `collate_supfusion` (dict with eeg, ecg, y)."""
...
def main() -> None:
    ...
    threshold = find_best_threshold_youden(model, val_loader, DEVICE)
    print(f"Validation threshold (Youden, P(class=1)): {threshold:.4f}")

    val_metrics = evaluate_binary_threshold(model, val_loader, threshold, DEVICE)
    test_metrics = evaluate_binary_threshold(model, test_loader, threshold, DEVICE)

    print(
        "Val metrics (at threshold):",
        {k: val_metrics[k] for k in ("auc_score", "accuracy", "f1", "precision", "recall") if k in val_metrics},
    )
    print(
        "Test metrics (at threshold):",
        {k: test_metrics[k] for k in ("auc_score", "accuracy", "f1", "precision", "recall") if k in test_metrics},
    )
    ...
    results_payload = {
        ...
        "val": {k: val_metrics[k] for k in ("auc_score", "accuracy", "f1", "precision", "recall")},
        "test": {k: test_metrics[k] for k in ("auc_score", "accuracy", "f1", "precision", "recall")},
